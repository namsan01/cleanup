...
// 체크 추가하기
export const postTodoCheck = async (todoId, loginedUserId) => {
  try {
...
// 일부내용 가져오기
export const getEdit = async (todoId, successGetEdit) => {
  try {
    const url = `${SERVER_URL}api/todo/edit?todoId=${todoId}`;

    const res = await axios.get(url);
    successGetEdit(res.data);
  } catch (error) {
    // 개발 중에만 활용. 실제 서비스에서는 경고창 마무리
